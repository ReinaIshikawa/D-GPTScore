...
# python evaluation/user_study/user_study_99_write.py
...
if __name__ == "__main__":
    ...
    if args.num == 0:
        ...
        user_list = [f"{args.num:02d}"]

    for user in user_list:
        old_df = user_study_reader(
            [user], gen_method_list, output_dir)

        print(old_df)

        os.makedirs(output_dir, exist_ok=True)
        output_path = os.path.join(output_dir, f"{user}", f"userStudy_{user}.xlsx")
        ...
        for mode in mode_list:
            ...
            for i, idx in enumerate(index_list):
                ...
                saved_dirs_cpy = ["01_CustomDiffusion", "05_Mix-of-Show", "06_DreamBooth"]
                random.shuffle(saved_dirs_cpy)
                saved_dirs_cpy = saved_dirs_cpy + ["02_OMG_lora","03_OMG_instantID", "04_fastcomposer"]
                for sdi, sd in enumerate(saved_dirs_cpy):
                    ...
                    if sd == "03_OMG_instantID" or sd == "04_fastcomposer" or sd == "02_OMG_lora":
                        filtered = old_df[
                            (old_df['method'] == sd) & 
                            (old_df['mode'] == mode) & 
                            (old_df['prompt_type'] == prompt_type) & 
                            (old_df['id_'] == str(idx))
                        ]
                        print(filtered)
                        ...
                        if not filtered.empty:
                            score = filtered.iloc[0, 4] 
                            add_text_ws1(score, row_counter+1, 2 + sdi, False)
                        else:
                            print(f"data not found（mode={mode}, prompt_type={prompt_type}, id_={id_}）")
